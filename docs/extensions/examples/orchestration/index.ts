...
// `eventSubscriptions: ["task:assignment_update"]`. No storage — the
// tool has no persistent state. Pending invocations live in a process-
// local map keyed on `assignmentId`; the subprocess is `persistent:
// true` so the map survives across calls.
