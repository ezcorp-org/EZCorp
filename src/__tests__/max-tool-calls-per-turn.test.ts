/**
 * Phase 6 (M3) — `MAX_TOOL_CALLS_PER_TURN=10` enforcement.
 *
 * Pre-Phase-6: the constant was declared at `tool-executor.ts:25` but
 * never checked. A misbehaving / compromised LLM could fan out 1000+
 * tool calls per turn before the run loop noticed.
 *
 * Phase 6 wires:
 *   - Per-conversation per-turn counter (process-singleton Map).
 *   - 11th call throws `MaxToolCallsExceededError`.
...
describe("MAX_TOOL_CALLS_PER_TURN enforcement (M3)", () => {
	test("constant equals 10 (spec lock-in)", () => {
		expect(MAX_TOOL_CALLS_PER_TURN).toBe(10);
	});

	test("first 10 calls in a single conversation succeed; 11th throws", async () => {
		...
		expect(_getToolCallsThisTurnForTests("conv-mtc-1")).toBe(10);

		// 11th throws.
		await expect(
			exec2.executeToolCall("ext-mtc2__do", {}, "conv-mtc-1", "msg-11"),
	...
	test("two parallel conversations don't share counters", async () => {
		...
		expect(_getToolCallsThisTurnForTests("conv-A")).toBe(10);
		expect(_getToolCallsThisTurnForTests("conv-B")).toBe(10);

		await expect(
			executor.executeToolCall("ext-mtc4__do", {}, "conv-A", "msg-X"),
		).rejects.toBeInstanceOf(MaxToolCallsExceededError);
		// conv-B is still at 10, not 11; it's at its limit but next
		// call hasn't been made.
		expect(_getToolCallsThisTurnForTests("conv-B")).toBe(10);
	});
