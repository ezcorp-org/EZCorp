...
describe("lessons queries", () => {
  ...
  describe("slug-scope uniqueness (partial unique indexes)", () => {
    ...
    test("project + global at the same slug collide (both share the project-scoped index)", async () => {
      await make({ slug: "ladder", visibility: "project" });
      await expect(
        make({ slug: "ladder", visibility: "global" }),
      ).rejects.toThrow();
    });
