...
export class ExtensionProcess {
  ...
  ensureRunning(): void {
    ...
    // Drain stderr so the kernel pipe buffer (~64KB on Linux) never fills.
    // Without this, any extension that writes enough to stderr — e.g. Bun
    // printing a preload resolution error, or an unhandled promise rejection
    // — would eventually block on write and surface as a spurious
    // "Transport closed" crash. Contents are discarded: extensions should
    // only use stdout for JSON-RPC framing; stderr is diagnostic only.
