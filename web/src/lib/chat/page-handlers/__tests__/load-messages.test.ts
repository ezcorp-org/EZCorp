...
describe("pathToRoot", () => {
	test("returns [] when the leaf id is not present", () => {
		expect(pathToRoot([makeMessage({ id: "x" })], "missing")).toEqual([]);
	});

	test("returns [] on an empty messages array", () => {
		expect(pathToRoot([], "any")).toEqual([]);
	});

	test("returns a single-element path for a root node", () => {
		const messages = [makeMessage({ id: "root" })];
		expect(pathToRoot(messages, "root").map((m) => m.id)).toEqual(["root"]);
	});

	test("walks parent links and returns messages in root → leaf order", () => {
		const messages = [
			makeMessage({ id: "m1", parentMessageId: null }),
			makeMessage({ id: "m2", parentMessageId: "m1" }),
			makeMessage({ id: "m3", parentMessageId: "m2" }),
			// Sibling branch that must NOT appear in the path to m3.
			makeMessage({ id: "m3b", parentMessageId: "m2" }),
		];
		expect(pathToRoot(messages, "m3").map((m) => m.id)).toEqual([
			"m1",
			"m2",
			"m3",
		]);
	...
	test("stops cleanly on a parent cycle (visited guard)", () => {
		const messages = [
			makeMessage({ id: "a", parentMessageId: "b" }),
			makeMessage({ id: "b", parentMessageId: "a" }),
		];
