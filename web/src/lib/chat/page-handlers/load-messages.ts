...
export function computeLatestLeaf(msgs: Message[]): string | null {
	...
	// `capability-event` rows are inline annotations persisted with a
	// null `parentMessageId` and no children — they are NOT conversational
	// tree nodes. If they're eligible as leaves, a trailing
	// capability-event (common right after an auto-allowed tool run)
	// becomes the active leaf; `pathToRoot` from it then yields a branch
	// containing no user/assistant messages and the thread renders BLANK.
	// Select the leaf only among real tree messages.
	const treeMsgs = msgs.filter((m) => m.role !== "capability-event");
	if (treeMsgs.length === 0) return null;
	const parentIds = new Set(msgs.map((m) => m.parentMessageId).filter(Boolean));
	const leaves = treeMsgs.filter((m) => !parentIds.has(m.id));
	if (leaves.length === 0) return treeMsgs[treeMsgs.length - 1]?.id ?? null;
 ...
 * isn't present. Shared by the chat thread's active-path `$derived` and
 * the branch-to-fork handler so both compute the same path.
 */
export function pathToRoot(messages: Message[], leafId: string): Message[] {
