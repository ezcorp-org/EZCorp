...
export function makeSendMessage(host: SendMessageHost): SendMessageHandlers {
	async function handleSend(
		...
		// Resolve activeLeafId — if it's a streaming placeholder, use its
		// real parent instead (placeholders are id'd `streaming-<runId>`
		// and never persisted). Without this, a follow-up message during
		// an active stream would parent itself to a soon-to-be-replaced
		// placeholder and break branch walking.
		...
		if (resolvedParentId?.startsWith("streaming-")) {
			const placeholder = allMessagesNow.find(
				(m) => m.id === resolvedParentId,
			);
