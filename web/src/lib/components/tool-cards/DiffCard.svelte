<script lang="ts">
	...
	let isNew = $derived(checkNewFile(details.oldContent, details.newContent));

	let diffText = $derived(generateDiffText(details.oldContent ?? '', details.newContent ?? '', filePath));
	...
	>
		...
		<div class="ml-auto flex items-center gap-1">
			{#if details.newContent}
				<CopyButton text={details.newContent} />
			{/if}
		</div>
	</button>
